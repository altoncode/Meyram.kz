...
/* ===== GAS endpoint (сіз берген "жаңа ключ") ===== */
...
function renderQuestion(){
  ...
  labels.forEach((lab, idx)=>{
    ...
    // ❗ ақ мәтін
    btn.style.color = '#fff';
    // қалауыңызша фонды да күшейтуге болады:
...
function move(d){
  ...
  if (current>=QUESTIONS.length){ finishQuiz(); return; } // ❗ серверде PDF жасап, содан кейін ғана нәтиже
  renderQuestion();
}
...
async function finishQuiz(){
  ...
  renderResultContent();         // 3) содан кейін ғана нақты нәтиже шығарамыз
  $('#progress').style.width='100%';
}

/* ===== Export / Send actions ===== */
// PDF-ті жаңа табта ашып, iframe жүктелген бойда print шығару (попап-блокерге ілінбейді)
function onExportPdf(){
  // 1) Батырма клигімен бірден таб аша саламыз (осы — попап-блокерді айналып өтуге себеп)
  const win = window.open('about:blank', '_blank', 'noopener');
  if (!win) {
    alert('Браузер попапты бұғаттады. Сайтқа pop-up рұқсат беріңіз.');
    return;
  }

  // 2) Асинхрон түрде серверден дайын PDF сілтемесін аламыз,
  //    дайын болғанда — сол ашық тұрған табқа viewer HTML жазамыз.
  (async () => {
    const pdf = await ensurePdfCreated(); // { url, id, name } немесе null
    if (!pdf || !pdf.url) {
      try { win.close(); } catch(_) {}
      alert('PDF әзір емес. Бір сәттен кейін қайталап көріңіз.');
      return;
    }

    // 3) iframe onload болғанда — дәл сол табта print терезесін шығарамыз
    const viewerHtml = `
...
<style>
  ...
  @media print { iframe { height: 100vh; } }
</style>
</head><body>
<iframe id="pdf" src="${pdf.url}#view=FitH"></iframe>
<script>
  (function(){
    var fr = document.getElementById('pdf');
    // iframe жүктелгенде print шақырамыз
    fr.addEventListener('load', function(){
      // кішкентай кідіріс кейбір браузерлер үшін тұрақтылық береді
      setTimeout(function(){
...
<\/script>
    ...
    try { win.close(); } catch(_) {}
    alert('Жүктеу кезінде қате. Кейінірек байқап көріңіз.');
  });
}
